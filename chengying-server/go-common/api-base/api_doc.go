...
func FilteredSQLInject(ctx context.Context) (ret bool) {
	errs := NewApiParameterErrors()
	defer func() {
		if r := recover(); r != nil {
			Feedback(ctx, r)
	...
	queryParams := ctx.URLParams()
	if strings.Contains(ctx.Path(), "inspect") {
		errs.CheckAndThrowApiParameterErrors()
		return true
	}

	//str := `(?:')|(?:--)|(/\\*(?:.|[\\n\\r])*?\\*/)|(\b(select|update|and|or|delete|insert|trancate|char|chr|into|substr|ascii|declare|exec|count|master|into|drop|execute)\b)`
	str := `(?i)((\s*|'|")\b(select|insert|update|delete|grant|revoke|create|drop|trancate|alter|and|or|\|\|)\b(\s*|'|"))`
