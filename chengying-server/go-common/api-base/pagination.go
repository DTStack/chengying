...
func GetPaginationFromQueryParameters(paramErrs *ApiParameterErrors, ctx context.Context, table interface{}) *Pagination {
	...
	if err := sortByAssert(sortBy, table); err != nil {
		sortBy = ""
	}
...
func sortByAssert(sortBy string, table interface{}) error {
	if sortBy == "" || table == nil {
		return nil
	}
	columns := utils.GetTagValues(table, "db")
	if inSlice(columns, sortBy) {
		return nil
	}
	return fmt.Errorf("invalid value %s", sortBy)
}
