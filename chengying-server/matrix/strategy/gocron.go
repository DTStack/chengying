...
// jasonlvhit@gmail.com https://github.com/jasonlvhit/gocron
...
func (j *Job) run() (result []reflect.Value, err error) {
	if j.lock {
		if locker == nil {
			err = fmt.Errorf("trying to lock %s with nil locker", j.jobKey)
			log.Errorf("err: %v", err.Error())
			return
		}

		if ok, err := locker.Lock(j.jobKey); err != nil || !ok {
			return nil, err
		}

		defer func() {
			if e := locker.Unlock(j.jobKey); e != nil {
				err = e
			}
		}()
	}
	...
	j.lastRun = time.Now()
	//log.Infof("call Job func: %v, param: %v", j.funcs[j.jobKey], j.fparams[j.jobKey])
	result, err = callJobFuncWithParams(j.funcs[j.jobKey], j.fparams[j.jobKey])
	j.scheduleNextRun()
	return
}
...
func getFunctionKey(id, funcName string) string {
	h := sha256.New()
	h.Write([]byte(id + "_" + funcName))
	return fmt.Sprintf("%x", h.Sum(nil))
}

// Do specifies the jobFunc that should be called every time the job runs
func (j *Job) Do(id string, jobFun interface{}, params ...interface{}) {
	typ := reflect.TypeOf(jobFun)
	if typ.Kind() != reflect.Func {
		log.Errorf("only function can be schedule into the job queue.")
		return
	}
	fkey := getFunctionKey(id, getFunctionName(jobFun))
	j.funcs[fkey] = jobFun
	j.fparams[fkey] = params
	j.jobKey = fkey
	//log.Infof("getFunction Jobkey: %v, JobName: %v, param: %v, interval: %v", j.jobKey, getFunctionName(jobFun), j.fparams[fkey],j.interval)

	j.scheduleNextRun()
}

// DoSafely does the same thing as Do, but logs unexpected panics, instead of unwinding them up the chain
func (j *Job) DoSafely(id string, jobFun interface{}, params ...interface{}) {
	recoveryWrapperFunc := func() {
		defer func() {
			if r := recover(); r != nil {
				log.Infof("Internal panic occurred: %v", r)
			}
		}()
...
func (j *Job) scheduleNextRun() {
	...
	//log.Infof("job %v next run time: %v", j.jobKey, j.nextRun)
}
...
// Remove specific job j by function key
func (s *Scheduler) Remove(id string, j interface{}) {
	i := 0
	key := getFunctionKey(id, getFunctionName(j))
...
func (s *Scheduler) Scheduled(id string, j interface{}) bool {
	for _, job := range s.jobs {
		if job.jobKey == getFunctionKey(id, getFunctionName(j)) {
			return true
		}
	}
	return false
...
func Remove(id string, j interface{}) {
	DefaultScheduler.Remove(id, j)
}

// Scheduled checks if specific job j was already added
func Scheduled(id string, j interface{}) bool {
