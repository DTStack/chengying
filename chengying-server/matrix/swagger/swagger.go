...
import (
	_ "dtstack.com/dtstack/easymatrix/matrix/swagger/doc"
...
func InitializeSwagger(app *iris.Application) {
	app.Get("/swagger", func(ctx context.Context) {
		ctx.Redirect("/swagger/index.html", iris.StatusMovedPermanently)
	})
	app.Get("/swagger/{any:path}", func(ctx context.Context) {
		WrapHandler(ctx.ResponseWriter(), ctx.Request())
	})
}
